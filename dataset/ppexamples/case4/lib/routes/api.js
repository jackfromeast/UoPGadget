...
router.patch('/*', (req, res, next) => {
  ...
  if (Array.isArray(parent)) {
    ...
    // Task 6 Patch: hasOwnProperty()
    if(check_pp_by_hasOwnProperty(req.body)){
      next({ code: 444, message: 'Never try to pollute my prototype!' });
      return undefined;
    }
    else if(path.join('.').indexOf('__proto__')!=-1||path.join('.').indexOf('prototype')!=-1){
      next({ code: 444, message: 'Never try to pollute my prototype!' });
      return undefined;
    }
...
router.post('/*', (req, res, next) => {
  ...
  let parent = ['store'];
  while (root) {
    // patch for task5-poc4
    if(root === '__proto__' || root === 'prototype'){
      next({ code: 444, message: 'Never try to pollute my prototype!' });
      return undefined;
    }
    if (!object[root]) {
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    if (!storePath) { // this means it's a root reset
      ...
      else if(storePath.indexOf('__proto__')!=-1||storePath.indexOf('prototype')!=-1){
        next({ code: 444, message: 'Never try to pollute my prototype!' });
        return undefined;
      }
...
function check_pp_by_hasOwnProperty(input_object){
  for (var i in input_object){
    if(input_object.hasOwnProperty('__proto__')||input_object.hasOwnProperty('prototype')){
      return true;
    }
    else if(is_object(input_object[i])){
      check_pp_by_hasOwnProperty(input_object[i]);
    }
  }
  return false;
}

function patch_pp_by_freeze(){
  Object.freeze(Object.prototype);
}

function patch_pp_by_unfreeze(){
  // trick for unfreeze an object
  Object.prototype = JSON.parse(JSON.stringify(Object.prototype));
}
