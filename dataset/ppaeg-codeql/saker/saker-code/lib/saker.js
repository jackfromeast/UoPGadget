 ...
 * Copyright(c) 2017 Sky <eshengsky@163.com>
 * MIT Licensed
 */
...
!function () {
    ...
    var configure = {
        ...
        /**
         * 开启Debug模式。Open debug mode.
         */
        debug: false,

        /**
         * 服务端脚本标记符号。Server-side scripts mark symbol.
         */
        symbol: '@',

        /**
         * 默认的母版页路径。Default layout file path.
         */
        defaultLayout: 'layout.html',

        /**
         * 默认的局部页所在目录。Default directory containing partial files.
         */
    ...
    ParseProcessor.prototype = {
                ...
                if (char === '>' && this.tags.length > 0) {
                    ...
                    if (new RegExp('<(\\\\)?\/' + this.tags[this.tags.length - 1].type + '\\s*>$').test(this.readPrevChars() + char) || this.selfClosedTags.indexOf(this.tags[this.tags.length - 1].type) > -1) {
                        ...
                        if ((this.tags[this.tags.length - 1] || { position: -1 }).position >= (this.braces[this.braces.length - 1] || { position: -1 }).position) {
                            ...
                            this.position += 1;
                            //如果接下来的字符是空格。If next char is whitespace.
        ...
        readBlockServerCode: function () {
            ...
            for (; this.position < len; this.position++) {
                char = this.readNextChars(1);
                if (char === configure.symbol) {
                    throw new SakerError('In block scripts, please write scripts directly without prefix ' + configure.symbol + ' !', this.getStackString('In block scripts, please write scripts directly without prefix ' + configure.symbol + ' !', this.getLineNum(this.position)));
                }
                //引号的处理。Handle quotes.
                if ((char === '"' || char === "'") && this.readPrevChars(1) !== '\\') {
                    if (this.quotes.length === 0) {
                ...
                if (nextChar === configure.symbol) {
                    ...
                    //@*...*@
                        ...
                        if (new RegExp('\\*[\\s\\S]*?\\*' + configure.symbol).test(processor.readNextChars())) {
                            ...
                            throw new SakerError('Comments ' + configure.symbol + '* not found the matched *' + configure.symbol + ' !', processor.getStackString('Comments ' + configure.symbol + '* not found the matched *' + configure.symbol + ' !', processor.getLineNum(processor.position)));
                        }
                    }
                    //@(...)
                    else if (nextChar === '(') {
                    ...
                    //@ abc, @", @?等其他特殊字符
                        ...
                        throw new SakerError('Illegal character after ' + configure.symbol + ' ! ', processor.getStackString('Illegal character after ' + configure.symbol + ' ! ', processor.getLineNum(processor.position)));
                    }
                } else {
                    code = processor.readBlockServerCode();
                    contentProcessor.addSegment({
