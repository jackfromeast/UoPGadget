...
class PropAccessUnderSrcPath extends PropAccess {
  ...
  PropAccessUnderSrcPath() { this.getFile().getAbsolutePath().indexOf("/home/PPAEG/work/")=0}
}

// limit the results from a given src path
class MethodCallExprUnderSrcPath extends MethodCallExpr {
  /* the characteristic predicate */
  MethodCallExprUnderSrcPath() { this.getFile().getAbsolutePath().indexOf("/home/PPAEG/work/")=0}
}
...
select prop_access, "$@", prop_access, prop_access.toString()