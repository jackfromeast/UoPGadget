...
function extractProps(){
    ...
    for (var codebaseName of codebaseNames){
        ...
        let outputPath = `../../output/silent-spring/extracted-props/${codebaseName}-extracted-props.json`
...
if (codebaseNames[0] === 'all') {
    const allCodebasesPath = '../../dataset/ppaeg-codeql';
    const allCodebases = fs.readdirSync(allCodebasesPath);
    const filteredCodebases = allCodebases.filter(codebase => fs.lstatSync(path.join(allCodebasesPath, codebase)).isDirectory());
    codebaseNames.splice(0, 1, ...filteredCodebases);
}
