...
async function findUndefined(propFilename, apiName) {
    try {
        let nodeExtractedPropertyPath = `../../output/silent-spring/extracted-props/${propFilename}-extracted-props.json`
        let resultPath = `../../output/silent-spring/undefined-props/${propFilename}-undefined-props.json`
...
function main(){
    if (extractedProps[0] === 'all') {
        for (const propFilename in exportedAPIs) {
            findUndefined(propFilename, propFilename);
        }
    } else {
