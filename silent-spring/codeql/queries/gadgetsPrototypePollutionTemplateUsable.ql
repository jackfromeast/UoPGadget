 ...
 */
 ...
 predicate undefinedPropertyNames(string propertyName) {
  propertyName = ["alias", "property", "__createBinding", "__setModuleDefault", "__importStar", "__importDefault", "pretty", "doctype", "inlineRuntimeFunctions", "debug", "escapePrettyMode", "filename", "h1", "xml", "code", "block", "globals", "self"]
}
  ...
  //  }
   ...
   override predicate isSink(DataFlow::Node node, DataFlow::FlowLabel lbl) {
     (node instanceof EvalJavaScriptSink
      or
      // also include key variable as sink
      // in case of some data flow gap
      node instanceof DataFlowNodeIsKeyVariable
     )
     and 
     (
